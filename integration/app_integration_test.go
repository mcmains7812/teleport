...
func TestAppServersHA(t *testing.T) {
	...
	for name, test := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			// returns error since thers is no app server for this app
			// status, err = test.makeRequest(pack, inCookie)
			// requestWithError(t, status, err)
			...
			for _, appServer := range servers {
				...
				// everytime a app server stops we issue an request to
...
func setupWithOptions(t *testing.T, opts appTestOptions) *pack {
	...
	p.rootAppServers = p.startRootAppServers(t, opts.rootAppServersCount, opts.extraRootApps)
...
func (p *pack) startRootAppServers(t *testing.T, count int, extraApps []service.App) []*service.TeleportProcess {
	...
	for i := 0; i < count; i++ {
		...
		raConf.Console = nil
		raConf.Log = log
		raConf.DataDir, err = ioutil.TempDir("", fmt.Sprintf("app-server-%d-%s", i, p.rootCluster.Secrets.SiteName))
		require.NoError(t, err)
