...
func Run(args []string, opts ...cliOption) error {
	...
	ssh.Flag("", "Setup x11 forwarding in untrusted mode (secure) for this request").Short('X').BoolVar(&cf.X11Forwarding)
	ssh.Flag("", "Setup x11 forwarding in trusted mode (insecure) for this request").Short('Y').BoolVar(&cf.X11ForwardingTrusted)
...
func makeClient(cf *CLIConf, useProfileLogin bool) (*client.TeleportClient, error) {
	...
	c.X11ForwardingTrusted = cf.X11Forwarding
